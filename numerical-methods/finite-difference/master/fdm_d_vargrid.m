...
%
% TODO merge with derivative_matrix_1d
...
function [L D] = fdm_d_vargrid(X,d,k)
    ...
    switch (k)
		...
		N_ = [ -(xc - xs).*(xc - xr).*(xc - xl), ...
                       (xc - xs).*(xc - xr).*(xc - xk), ...
                       4.*xc.^3 - 3.*xc.^2.*xl - 3.*xc.^2.*xr - 3.*xc.^2.*xs ...
			- 3.*xc.^2.*xk + 2.*xc.*xk.*xl + 2.*xc.*xk.*xr ...
			+ 2.*xc.*xk.*xs + 2.*xc.*xl.*xr + 2.*xc.*xl.*xs ...
			+ 2.*xc.*xr.*xs - xk.*xl.*xr - xk.*xl.*xs - xk.*xr.*xs - xl.*xr.*xs, ...
			-(xc - xs).*(xc - xl).*(xc - xk), ...
			-(xc - xr).*(xc - xl).*(xc - xk)
			];
		N = [	- ( xc.^2.*xl + xc.^2.*xr + xc.^2.*xs - xc.^3 - xc.*xl.*xr - xc.*xl.*xs - xc.*xr.*xs + xl.*xr.*xs ), ...
			- (xc.^3 - xc.^2.*xr - xc.^2.*xs - xc.^2.*xk + xc.*xk.*xr + xc.*xk.*xs + xc.*xr.*xs - xk.*xr.*xs ), ...
                       4.*xc.^3 - 3.*xc.^2.*xl - 3.*xc.^2.*xr - 3.*xc.^2.*xs ...
			- 3.*xc.^2.*xk + 2.*xc.*xk.*xl + 2.*xc.*xk.*xr + 2.*xc.*xk.*xs ...
			+ 2.*xc.*xl.*xr + 2.*xc.*xl.*xs + 2.*xc.*xr.*xs - xk.*xl.*xr - xk.*xl.*xs - xk.*xr.*xs - xl.*xr.*xs, ...
