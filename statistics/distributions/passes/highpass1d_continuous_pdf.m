...
%
%%
%% function [S_bp,Sc] = spectral_density_highpass_continous(fx,fc,order,normalize,pp)
%%
%% output :
%% S_bp : spectral density of the highpass filter in continuos space
%%     limit case of the discrete highpass for dx -> 0
...
function [S_bp,Sc] = highpass1d_continuous_pdf(fx,fc,order,normalize,pp)
...
%	else
		% lowpass density
		S_lp1 = lowpass1d_continuous_pdf(fx,fc,1,false);
		% transfer function
		T_lp1 = sqrt(S_lp1);
		% highpass density
		S_bp     = (1-S_lp1).^order;
		%T_lp1.^pp(1)).^pp(2).*T_lp1.^pp(3)).^2;
%	end
	...
	switch (normalize)
		...
		otherwise % analytic, for limit case df = 1/L = 0
			%Sc = highpass1d_continuous_pdf_scale(fc, order, pp);
			%S_bp = Sc.*S_bp;
	end % switch
end % spectral_density_highpass_continuous
