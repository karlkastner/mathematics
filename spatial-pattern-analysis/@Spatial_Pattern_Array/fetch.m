...
function obj = fetch(obj,inshpname,outshpname)
		...
		% fetch patterns that are not yet well resolved at higher resolution
		...
		for idx=1:obj.n
			t0 = toc(timer);
			% only process, if not yet fully resolved at the current level
			...
			shp(idx).dx_sample = obj.dx_sample(idx);
			catch e
				disp(e);
